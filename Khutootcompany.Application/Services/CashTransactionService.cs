...
namespace Khutootcompany.Application.Services
{
    public class CashTransactionService : ICashTransactionService
    {
        ...
        public async Task<CashTransactionDto> UpdateTransactionAsync(CashTransactionDto dto, string username)
        {
            var transaction = await _unitOfWork.CashTransactions.GetByIdAsync(dto.TransactionId);
            if (transaction == null)
